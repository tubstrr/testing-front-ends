<script setup>
  const { data } = await useFetch("https://jonknoll.dev/wp-json/todo/v1/todos");
  ...
  const addTodo = async () => {
    const add = await fetch(
      `https://jonknoll.dev/wp-json/todo/v1/add/?title=${newTodo.value}`
    ).then((res) => res.json());
    todos.value = [
      { id: add.id, title: add.title, done: false },
      ...todos.value,
    ];
    newTodo.value = "";
  };
  ...
  <section class="page">
    ...
    <ul>
      <li v-for="todo in todos" :key="todo.id">
        <button @click="updateTodo(todo.id)" class="done">
          <svg
            v-if="todo.done"
